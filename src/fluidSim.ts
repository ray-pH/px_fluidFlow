...
class FluidSimulator {
    ...
    GDye     : Float32Array;
    BDye     : Float32Array;
    RDyeprev : Float32Array;
    GDyeprev : Float32Array;
    ...
    avgVx(i : number, j : number, Vx : Float32Array) {
        let nx = this.nx;
        return (Vx[i   + nx*(j-1)] + Vx[i   + nx*j] +
                Vx[i+1 + nx*(j-1)] + Vx[i+1 + nx*j]) * 0.25;
    }

    avgVy(i : number, j : number, Vy : Float32Array){
        let nx = this.nx;
...
class FluidRenderer{
    ...
    constructor(fluidsim : FluidSimulator, canvas : HTMLCanvasElement){
    ...
    draw(ro : RenderOption) {
        ...
        for (let i = 0; i < fl.n; i++){
            ...
            if (fl.S[i] == 0 && ro['show_obstacle']) color = obs_color.slice();
        ...
        if (ro['show_streamline']) this.drawStreamline();
    }
